 ...
 */
class melanie2_logs extends rcube_plugin
{
	const DEBUG = 'DEBUG';
	const INFO = 'INFO';
	const ERROR = 'ERROR';
	...
	function __construct($api) {
	    ...
	    $this->log_level = explode('|', rcmail::get_instance()->config->get('melanie2_logs_level'));
	}
	...
	/**
	 * Test si l'instance de melanie2_log permet de logger a ce niveau
   ...
   */
	...
	public function log($level, $message)
	{
	    if (in_array($level, $this->log_level)) {
	        ...
	        rcmail::get_instance()->write_log($this->log_file, "[$level] $ip PROC[$procid] $username - $message");
	    }
	}
