		...
		if (onoff == true)
		{
			...
			if (header_preview_all_table.find('tr td.label').length > 0) {
				...
				if (header_preview_all_table.length > 0) {
					header_preview_all_table.append('<tr><td class="header-title">'+rcmail.gettext('melanie2_labels.labels')+'</td><td class="header label"><span class="label_' + toggle_label.replace('~', '') + '">' + label_name + '</span></td></tr>');
				}
			}
		}
		else
		...
		{				
			...
			if (rowobj.find('td.labels').length > 0) {
				if (rowobj.find('td.labels').text() == "") {
					...
					rowobj.find('td.labels').text(rowobj.find('th.labels').text() + ', ' + label_name);
				}
			}
...
$(document).ready(function() {
	...
	if ($('#listoptions').length > 0) {
		$('#listoptions fieldset').first().find('ul').append('<li><label><input type="checkbox" name="list_col[]" value="labels" /> <span>'+rcmail.gettext('melanie2_labels.labels')+'</span></label></li>');
	}
	if ($('th#rcmlabels').length > 0) {
		$('th#rcmlabels span.labels').text(rcmail.gettext('melanie2_labels.labels'));
	}	
	...
	rcmail.addEventListener('listupdate', function() {
		if ($('td#rcmlabels').length > 0) {
			$('td#rcmlabels span.labels').text(rcmail.gettext('melanie2_labels.labels'));
		}
	});
